...
def create_table(criteria):
    ...
    for i1 in criteria['function_name']:
        for i2 in criteria['swarm_distribution']:
            for i3 in criteria['phi_distribution']:
                for i4 in criteria['experiment']:
                    criteria_arr = [i1, i2, i3, i3, i4]
                    for file in files:
...
def create_boxplot(df, criteria):
    ...
    if criteria['phi_distribution'] is not None:
        criteria_list = np.array(df['Criteria'].unique())
        index_list = []
